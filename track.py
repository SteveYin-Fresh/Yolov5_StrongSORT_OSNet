...
def run(
    ...
    exp_name = (
        ...
        + "_"
        + str(strong_sort_weights).split("/")[-1].split(".")[0]
    )
            ...
            if det is not None and len(det):
                ...
                if len(outputs[i]) > 0:
                    for j, (output, conf) in enumerate(zip(outputs[i], confs)):
                        ...
                        if (
                            save_vid or save_crop or show_vid
                        ):  # Add bbox to image
                ...
                LOGGER.info(
                    f"{s}Done. YOLO:({t3 - t2:.3f}s), \
                        StrongSORT:({t5 - t4:.3f}s)"
                )
    ...
    LOGGER.info(
        f"Speed: %.1fms pre-process, %.1fms inference, %.1fms NMS, %.1fms \
            strong sort update per image at shape {(1, 3, *imgsz)}"
        % t
    )
    if save_txt or save_vid:
        s = (
            f"\n{len(list(save_dir.glob('tracks/*.txt')))} tracks saved to \
                {save_dir / 'tracks'}"
            if save_txt
            else ""
        )
...
def parse_opt():
    ...
    parser.add_argument(
        ...
        type=str,
        default="0",
        help="file/dir/URL/glob, 0/1/2 for webcam",
    )
    parser.add_argument(
        "--source-width", type=int, default=640, help="webcam frame width"
    )
    parser.add_argument(
        "--source-height", type=int, default=480, help="webcam frame height"
    )
    parser.add_argument(
        "--source-flip", action="store_true", help="webcam frame mirror flip"
    )
    parser.add_argument(
        "--imgsz",
        "--img",
